...
function normalizeCaseInsensitiveActionStringAlias(action, canonicalKey, label) {
  const matches = Object.keys(action).filter((k) => k.toLowerCase() === canonicalKey.toLowerCase());
  if (matches.length === 0) {
    return;
  }
  let resolved;
  for (const key of matches) {
    ...
    if (typeof raw === "string" && raw.trim() !== "") {
      value = raw;
    } else if (Array.isArray(raw) && raw.length === 1 && typeof raw[0] === "string" && raw[0].trim() !== "") {
      value = raw[0];
    } else {
      throw new Error(
        `${label}.${key}: expected a non-blank string (a one-element array of one non-blank string is accepted as an alias); got ${JSON.stringify(raw)}`
      );
    }
    if (resolved !== void 0 && resolved !== value) {
      throw new Error(
        `${label}: multiple case variants of "${canonicalKey}" were given and disagree (found ${matches.map((k) => JSON.stringify(k)).join(", ")}) -- an ambiguous combined input is refused rather than silently resolved one way`
      );
    }
    resolved = value;
  }
...
function requirePositiveInt(value, label) {
  if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
    throw new Error(`${label}: expected a positive integer; got ${JSON.stringify(value)}`);
  }
  return value;
}
function requireTimeConditionFieldsForType(condition, ordinal, label) {
  ...
  if (ordinal === T.EVERY_DAY && condition.weekdayOnly !== true || ordinal === T.EVERY_WEEK || ordinal === T.DAY_OF_WEEK) {
    condition.interval = condition.interval === void 0 || condition.interval === null ? 1 : requirePositiveInt(condition.interval, `${label}.interval`);
  }
...
function readScheduleChange(raw, index) {
  ...
  if (Array.isArray(actions)) {
    actions.forEach((action, actionIndex) => {
      ...
      normalizeCaseInsensitiveActionStringAlias(action, "ccaddresses", actionLabel);
      normalizeCaseInsensitiveActionStringAlias(action, "bccaddresses", actionLabel);
